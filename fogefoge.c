...
int paraondeofantamavai(int xatual, int yatual,
    ...
    for(int i = 0; i < m.linhas; i++) {
        for(int j = 0; j < m.colunas; j++) {
            ...
            if(copia.matriz[i][j] == FANTASMA) {
                ...
                int encontrou = paraondeofantamavai(i, j, &xdestino, &ydestino);
...
void move(char direcao) {
    ...
    if(!direcaovalida(direcao)) return;
...
void explodepilula2(int x, int y, int somax, int somay, int qtd) {
	...
	if(!direcaopermitida(&m, novox, novoy)) return;
